...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "Moraine is down; start it from a host-level service, not a session sandbox." >&2
  exit 1
fi
...
if launch_running clickhouse && launch_running ingest && launch_running backend; then
  echo "Moraine launch agents are running, but host health is not visible." >&2
  echo "Rerun this health check with host-level escalation; do not restart healthy services from a sandbox result." >&2
  exit 2
fi
...
echo "Moraine launch agents were kicked, but the stack is still unhealthy." >&2
